export type AchRecord = {
  ...
  count: number;         // jumps in one session
  duration: number;      // seconds
  score10?: number;      // 0-10
  symmetry_score?: number; // 0-100 (optional, fallback to score10/convert)
  misses?: number;       // count of misses in session
};
...
export type PlanConfig = {
  targetWeeklyHours?: number; // default 5 hours
};

const clamp01 = (x:number) => Math.max(0, Math.min(1, x));
const sum = (arr:number[]) => arr.reduce((s,x)=> s+x, 0);
const byTsAsc = (a:AchRecord,b:AchRecord)=> (a.ts||0)-(b.ts||0);
...
export function calculateLevel(totalExp:number){
  let lvl = 0; let expLeft = totalExp;
  while (true) {
    const need = expForLevel(lvl+1);
    if (expLeft >= need) { lvl++; expLeft -= need; } else break;
    if (lvl > 1000) break; // safety
  }
  const nextNeed = expForLevel(lvl+1);
  return { level: lvl, expIntoLevel: expLeft, nextLevelNeed: nextNeed };
}

function stats(records:AchRecord[]) {
  ...
  const misses = recs.map(r=> Number(r.misses)||0);
  ...
  // 连续8分+次数
  let consec8 = 0, bestConsec8 = 0;
  for (const s of scores) { if (s>=8) { consec8++; bestConsec8=Math.max(bestConsec8,consec8);} else consec8=0; }

  // 2分以上快速提升（相邻两次评分差>=2）
  let rapidGain = false;
  for (let i=1;i<scores.length;i++){ if ((scores[i]-scores[i-1])>=2) { rapidGain = true; break; } }
  ...
  const byDay = new Map<string, {duration:number; count:number; scores:number[]}>();
  ...
  // 任意7天/30天连续满勤
  function longestStreakDays():number{
    if (recs.length===0) return 0;
    const keys = Array.from(byDay.keys()).sort();
    let best=0, curr=0, prevKey='';
    for (const k of keys){
      if (prevKey==='') { curr=1; } else {
        const [y,m,d] = prevKey.split('-').map(Number);
        const prevDate = new Date(y,m-1,d);
        prevDate.setDate(prevDate.getDate()+1);
        const nextKey = toDayKey(prevDate.getTime());
        curr = (nextKey===k) ? (curr+1) : 1;
      }
      best = Math.max(best,curr); prevKey=k;
    }
    return best;
  }
...
export type AchievementDef = (typeof ACHIEVEMENTS_DEF)[number];

export function evaluateAchievements(records:AchRecord[], plan:PlanConfig={}){
  const cfg = { targetWeeklyHours: 5, ...plan };
  const s = stats(records);
  const res: Achievement[] = ACHIEVEMENTS_DEF.map(def=> ({...def, unlocked:false, progress:0}));

  const setState = (id:string, unlocked:boolean, progress:number, unlockedAt?:number)=>{
    const item = res.find(a=> a.id===id)!; item.unlocked = unlocked; item.progress = clamp01(progress); if (unlocked && unlockedAt) item.unlockedAt = unlockedAt;
  };
...
export function updateAchievements(prev:Achievement[]|undefined, records:AchRecord[], plan?:PlanConfig){
  const { achievements } = evaluateAchievements(records, plan);
  // 保留已解锁的时间戳
  const merged = achievements.map(a=>{
    const old = prev?.find(p=> p.id===a.id);
    if (a.unlocked) {
      return { ...a, unlockedAt: a.unlockedAt ?? old?.unlockedAt ?? (records.length? records[0].ts: undefined) };
    } else {
      return { ...a, unlockedAt: old?.unlockedAt };
    }
  });
